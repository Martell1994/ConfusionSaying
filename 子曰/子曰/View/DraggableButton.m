...
#define WAITING_KEYWINDOW_AVAILABLE 0.f
#define AUTODOCKING_ANIMATE_DURATION 0.2f
#define DOUBLE_TAP_TIME_INTERVAL 0.36f
...
- (id)initInKeyWindowWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        [self performSelector:@selector(addButtonToKeyWindow) withObject:nil afterDelay:WAITING_KEYWINDOW_AVAILABLE];
        [self defaultSetting];
    }
    return self;
}
...
- (void)defaultSetting {
    ...
    //[self.layer setBorderColor:kRGBColor(176, 166, 167).CGColor];
//    [self.layer setBorderColor:kRGBColor(88, 88, 88).CGColor];
...
- (void)setTapBlock:(void (^)(DraggableButton *))tapBlock {
    _tapBlock = tapBlock;
...
#pragma mark - Touch
- (void)buttonTouched {
    [self performSelector:@selector(executeButtonTouchedBlock) withObject:nil afterDelay:(_doubleTapBlock ? DOUBLE_TAP_TIME_INTERVAL : 0)];
}
...
+ (void)removeAllFromKeyWindow {
    for (id view in [[UIApplication sharedApplication].keyWindow subviews]) {
        if ([view isKindOfClass:[DraggableButton class]]) {
